...
export default function Stats(props) {
    ...
    return (
            ...
            <TableBody>            
                {
                    props.parent.state.ranges ? 
                        props.parent.state.ranges.map(x => 
                            (  
                                <TableRow style={{backgroundColor: (props.parent.state.percentages[x.category] <= x.high) ? '#60ad5e' : '#f05545' }}>
                                  <TableCell>
                                      {x.category}
                                  </TableCell>
                                  <TableCell>
                                  ...
                                  <TableCell>
                                      <IconButton id={`${x.category}-delete`} onClick={deleteCategory}>
                                          <DeleteIcon fontSize="small"/>
                                      </IconButton>
                                  </TableCell>
                                  <TableCell>
                                      <IconButton id={x.category} onClick={editCategory}>
        ...
        <EditCategory closer={closeEditDlg}
           updateCategoryName={updateCategoryNameVal}
           updateLow={updateLowVal}
           updateHigh={updateHighVal}
           submitChange={updateCategoryRange}
           open={openEditDlg} 
           record={{category: currentCategory, low: currentLow, high: currentHigh}} />
        <EditCategory closer={closeNewCategoryDlg}
           updateCategoryName={updateCategoryNameVal}
           updateLow={updateLowVal}
           updateHigh={updateHighVal}
