...
class TransactionTable extends React.Component {
    ...
    doDataPull = async () => {
        ...
        if (resp.status === 200) {
            ...
            for (let cat in totals) {
                ...
                let percentage = (
                    (totals[cat] / totals["Income"]) *
                    100
                ).toFixed(2);
                percentages[cat] = percentage;
            }
            this.setState({
                activeCategories: this.initial
                    ? categoriesData
                    : this.state.activeCategories,
    ...
    deleteTrans = async (event) => {
        if (
            ...
            let resp = await fetch(
                `${this.host}/transactions/remove/` + event.currentTarget.id,
                {
                    method: "DELETE",
                }
            );
        ...
        if (data.oldName !== data.newName) {
            ...
            if (data.newName == "Income") {
                this.openFailureToast(
                    "Can't use 'Income' for a new category name!"
                );
                return;
            }

            let resp = await fetch(
                `${this.host}/categories/rename/` +
                    data.oldName +
                    "/" +
                    data.newName,
                {
    ...
    updateCurrentRecordCategory = (category) => {
        this.setState({
            currentRecord: { ...this.state.currentRecord, category: category },
        });
    };
    updateCurrentRecordDate = (date) => {
        this.setState({
            currentRecord: { ...this.state.currentRecord, date: date },
        });
    };
    updateCurrentRecordAmount = (amount) => {
        this.setState({
            currentRecord: { ...this.state.currentRecord, amount: amount },
        });
    };
    updateCurrentRecordRemarks = (remarks) => {
        this.setState({
            currentRecord: { ...this.state.currentRecord, remarks: remarks },
        });
    };
    closeEditDlg = () => {
    ...
    toggleAllCats = (selectAll) => {
        if (selectAll)
            this.setState({ activeCategories: [...this.state.categories] });
